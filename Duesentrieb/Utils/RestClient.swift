...
struct RestClient {
    private static let printJsonDecoding = false
    ...
    static func modelForRequest<T>(method: HttpMethod, url: URL, header: Header, session: URLSession) -> AnyPublisher<T, BackendError> where T: Codable {
        ...
        return urlDataTask(request: request, session: session)
            ...
            .map        { RestClient.printJsonString(data: $0) }
    ...
    static func modelForRequest<T, P>(data: P, method: HttpMethod, url: URL, header: Header, session: URLSession) -> AnyPublisher<T, BackendError> where T: Codable, P: Codable {
        return Just(data)
            ...
            .map        { RestClient.printJsonString(data: $0) }
            .decode(type: T.self, decoder: decoder())
            .mapError   { error -> BackendError in print(error.localizedDescription); return mapErrorIfNeeded(error, to: .jsonDecodingError) }
            .eraseToAnyPublisher()
    }
    ...
    private static func printJsonString(data: Data) -> Data {
...
//        backendError.localizedDescription = error.localizedDescription
        return backendError
    }
}
